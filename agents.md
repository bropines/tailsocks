...
* **Hostinfo masquerade (patch 06):** by default the daemon tells the coordination server `OS = linux`, `App = tailscale-cli`, `DeviceModel = Tailsocks`. It was added on 2026-05-07 on the belief that the coordinator ignored advertised services from Android nodes; that was never measured, nothing in the client reads these fields for behaviour, and Serve/Funnel work here because of patch 05 (the `!android` build tag on `ipn/localapi/cert.go`), not because of this. *Report the real OS* (Settings → Account & connection, off by default) sets `TS_HONEST_HOSTINFO=1` and the daemon reports `OS = android` with the real model, Android version and install source, the way the official client does. The knob is read at daemon start, so the switch restarts the daemon; the root env file carries it too. **Measured 2026-09-07 on the Redmi:** the coordination server refuses a *registered* node whose OS changes — health says «node OS changed since last connection, was node state copied between devices?», no netmap, no capabilities, `tailscale cert` fails — so the switch only works together with a fresh login (a new node identity), and its confirmation dialog says so. Whether a node registered as Android from the start receives `https`/`funnel` is still unmeasured; the masked node does, and Funnel and the `svc:` c2n pull were verified end to end.
