...
* **Root Mode rules live in named chains**: `TAILSOCKS_MARK` (mangle) and `TAILSOCKS_DNS` (nat), steering by masked fwmark `0x1000000/0x1000000` into table `53`. Never write a bare `--set-mark`. Put MARK/DNAT rules only inside `TAILSOCKS_MARK` / `TAILSOCKS_DNS`; `OUTPUT` carries only the jump into those chains (guarded with `-C`), and `FORWARD` only the `tailscale0` ACCEPT pair. Treat peer names and settings fields as untrusted before they reach a root shell (`RootUtils.shQuote`).
...
* **Open handoff documents** (self-contained briefs for a fresh session or another model): [`docs/HANDOFF_4.0_NATIVE_TUN.md`](docs/HANDOFF_4.0_NATIVE_TUN.md) — TUN mode rebuilt around a tailscaled-owned VpnService fd; research appendix in `docs/research/`. [`docs/HANDOFF_BACKLOG.md`](docs/HANDOFF_BACKLOG.md) — what is left of the backlog: Root Mode verification on real devices (WSA reboot autostart, Redmi/APatch) and making the CLI binary optional (needs the author's decision). Each task carries evidence references and an acceptance test; mark it done in the topic file when you finish it. When you finish or abandon a handoff, update or delete the document — a stale handoff is worse than none.
