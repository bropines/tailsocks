...
* **Passive Management**: Trust the Tailscale daemon to manage its own lifecycle, policy synchronization, and connectivity recovery. Do not write aggressive, high-frequency configuration loops or watchdog restarts. The only sanctioned recovery paths are the opt-in ones in `core/ServiceWatchdog.kt` (a 15-minute alarm — exact when *Alarms & reminders* is granted, inexact otherwise — that revives a killed service) and the auto-reconnect check in `TailscaledService` (three unhealthy ticks, bounded attempts, never while the daemon waits for login). Both are cancelled by a manual Stop, which is final — never add code that restarts the service after `desired_running` was cleared. A daemon that exits on its own is *not* a Stop: `onDaemonExited` keeps `desired_running` set and lets those two paths decide what happens next; only when neither may act does the service stand down, with a tap-to-reconnect notification.
...
* **Hostinfo masquerade (patch 06):** by default the daemon tells the coordination server `OS = linux`, `App = tailscale-cli`, `DeviceModel = Tailsocks`. It was added on 2026-05-07 on the belief that the coordinator ignored advertised services from Android nodes; that was never measured, nothing in the client reads these fields for behaviour, and Serve/Funnel work here because of patch 05 (the `!android` build tag on `ipn/localapi/cert.go`), not because of this. *Report the real OS* is a **profile** property (`honest_hostinfo` in `appctr_<id>`, `core/ProfileHostinfo.kt`), chosen when the profile is created — onboarding login step and the *Add account* dialog — and sets `TS_HONEST_HOSTINFO=1` so the daemon reports `OS = android` with the real model, Android version and install source, the way the official client does. It is per profile because **the coordination server binds a node to the OS it registered with** (measured 2026-09-07 on the Redmi: flipping it on a registered node gives health «node OS changed since last connection, was node state copied between devices?», no netmap, no capabilities). The Settings switch therefore changes an unregistered profile freely and, for a registered one (`ProfileHostinfo.isRegistered`: `was_logged_in` or a `NodeID` in the state file), offers a new profile instead; the service surfaces the refusal if it ever happens. Also measured: a node registered as Android from the start receives the same `https`/`funnel` capabilities, gets certificates, serves Funnel from the public internet, and the console names it after the device model. The knob is read at daemon start, so any change restarts the daemon; the root env file carries it too.
...
* **The boot script installs the routing half of T1 and nothing else.** At `late_start` there is no `PackageManager`, so it cannot resolve packages to uids and cannot install the exclusions and LAN `throw` routes that make T2/T3 safe — and no other VPN client has started yet, so a coexistence check there would answer "the device is free" every time. `/etc/hosts` publication is not in it either: the MagicDNS host entries are written by `RootUtils.updateRootHosts` from the app's peer-refresh tick, which needs a peer list and therefore an app process. The app installs the rest of T1 and all of T2/T3 on its first `Running` tick. State the cost plainly wherever it matters: with *Keep the connection awake* off, a reboot leaves no exit node and no device-wide MagicDNS until the app is opened, and tailnet *addresses* are reachable while tailnet *names* are not.
* **The boot script starts the daemon the way the app does, from the root-owned env file.** `/data/adb/tailsocks/control_proxy.env` is written by `RootUtils.writeRootEnvFile` on every Root Mode start (the app's own launch and the attach to a boot-started daemon) and carries, besides the proxy and daemon variables, the active profile's state directory, the SOCKS5/HTTP listen addresses, the SOCKS5 credentials and the tunnel mode. The script *parses* it and accepts only `export NAME='value'` lines — every value must go through `shQuote`; an unquoted line is silently dropped, which is how *Ignore other VPNs* reverted after every reboot up to 4.1.0. Both launch paths set the same modes: log 644, socket 666, state directory 700.
