...
### Changed
- **Settings were rebuilt from six tabs into a list of sections.** Settings now opens as a plain list — Account & connection, Tunnel mode, Local proxies, DNS, Censorship bypass, Sharing & access, Background & permissions, Appearance & language, Backup & restore, Automation & API, Diagnostics & developer — and tapping one opens only that page, so you can see every category at once instead of scrolling a strip of six chips.
- **Things that belong together are together now.** Proxy, VPN (TUN) and Root became one three-way switch under Tunnel mode, instead of a switch in one tab and a whole tab somewhere else; DNS options that were split across three tabs sit under DNS; ByeDPI moved next to the control-plane proxy it overrides; "Accept routes" and the device name joined the account, log verbosity and extra arguments joined Diagnostics, and the two separate backup/restore pairs became one Backup & restore section. Taildrive shares, Serve & Funnel, DNS diagnostics, Logs, Terminal, Netcheck and the Admin Console can now be opened straight from the section they belong to, and the battery-optimization row was dropped from Settings — it lives on the Permissions screen with the rest of them.
...
- **Service did not come back after an update** unless "Keep running in background" was on: installing a new version kills the running service, and the package-replaced handler only restarted it when that option was set. It now resumes whenever the service was running before the install; the option still decides only whether it also starts after a reboot.
...
- **Root Mode: exit nodes work, router health warning gone.** Selecting an exit node in Root Mode either did nothing or looped the tunnel's own traffic into `tailscale0`, and the daemon logged a permanent `router config failed` health warning: the desktop-Linux router was compiled in unchanged, with rules that assume a populated `main` table, netfilter on kernels that lack the nft `nat` chain, and marks that collide with Android's own. The daemon now marks its sockets (`0x2000000`) and only manages addresses and routes (table 52); the app routes unmarked local traffic through that table with a single rule (`fwmark 0x0/0x2020000 iif lo lookup 52`, pref 200) that it installs only once the daemon has logged that it marks its sockets — and only while no other VPN app holds the device — purges stale desktop rules, and loosens `rp_filter` when needed. Subnet routes accepted with `--accept-routes` work in Root Mode for the same reason, and **Check Routing** now shows the complete rule set and the daemon's table.
