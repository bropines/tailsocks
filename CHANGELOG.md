...
- **Permissions screen** (Settings → System & Backup → Permissions): every optional permission in one list — autostart, Alarms & reminders, battery optimization, notifications, installing unknown apps — each with why it matters, whether it is granted, and a button to the system screen that grants it. The status is rechecked every time you come back from those screens. Autostart is the one Android refuses to report, so it is shown as unknown rather than guessed at.
