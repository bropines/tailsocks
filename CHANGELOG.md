...
- **Root Mode no longer takes the device away from another VPN app.** Root Mode routes below Android's own VPN plumbing, so its two device-wide pieces — the rule that sends everything this phone sends through the exit node, and the redirection of every port-53 packet into MagicDNS — were installed even when another VPN client already held the device. Measured with a tunnel client running: its browser went dark, because its apps' packets were taken into the Tailscale exit node before the system's rules for that tunnel were ever consulted, and its apps' name resolution was taken as well, arriving at MagicDNS carrying the other tunnel's address. TailSocks now yields both of those to whoever holds Android's VPN slot and keeps only tailnet reachability: the node stays connected, tailnet addresses work from every app, MagicDNS names still resolve through the host entries Root Mode publishes, and the local SOCKS5 and HTTP proxies keep working — with exit-node egress for whatever is pointed at them, which is how another client can have the tailnet inside its own tunnel. What is given up while the other client claims every app on the phone is the exit node and system-wide MagicDNS for the rest of the device. The verdict and the reason behind it appear in the Logs screen and in Check Routing, the decision is re-made whenever another client starts or stops instead of once per connection, and rules an earlier run left behind are actively removed rather than merely not re-added. **Take the device anyway** (Tunnel mode) forces the full ruleset back for those who want it; the confirmation names what it costs the other app.
- **Root Mode now shares a phone with another VPN app, instead of only stepping aside.** Most tunnel clients do not take every app: whatever their owner puts on a bypass or per-app list is handed straight to Wi-Fi or mobile data, and belongs to nobody. TailSocks now carries exactly those apps — they follow the exit node and the accepted subnet routes as if nothing else were running — while the other client's own apps stay in its tunnel, untouched. Which apps those are is read from Android's own routing rules rather than guessed, and it is re-read whenever that client changes its list. Two kinds of app are never carried: another VPN app itself (its connection to its own server must not go through our exit node — and it is always outside its own tunnel, so without this rule the full yield could never happen at all), and identities no installed app owns. Name resolution is split the same way: the whole device keeps MagicDNS except the queries made for the other tunnel, which are recognised by the network they belong to — the only property that survives, since an ordinary app's lookups are sent by Android's resolver and not by the app. If that client leaves nothing out, the yield stays what it was. The dashboard, the Logs screen and Check Routing name which of the three cases this device is in.
- **Root Mode after a reboot no longer claims the device it cannot make room on.** The boot script used to install the device-wide half of Root Mode by itself — everything through the exit node, every port-53 packet into MagicDNS — but it runs before Android's package service exists, so it could install none of the exceptions that make those safe: no local-network carve-outs, no excluded apps, and no way to notice another VPN app (which has not started that early either). A reboot with an exit node selected therefore lost the local network again — the router, a NAS, a printer, adb over Wi-Fi — and the apps on the excluded list were not excluded, until the app was opened, or forever if it never was. The boot script now installs tailnet reachability only, and the app installs the rest together with its exceptions the moment it connects. **The cost, plainly:** with **Keep the connection awake** (Settings → Background & permissions) off, the app does not start at boot, so after a reboot there is no exit node and no system-wide MagicDNS until you open TailSocks — the node is up and your tailnet is reachable, but nothing else on the device goes through it. With it on, the gap is the few seconds until the app connects. This also removes two rules that had to be repeated word for word in two places at once: a change of wording in one of them silently switched exit nodes off in the other, with nothing to say why.
- **Root Mode's device-wide DNS redirect is armed only while MagicDNS is actually answering.** Redirecting every app on the phone to a resolver that has stopped answering leaves the device with no DNS at all, and the daemon does not report that case as a fault — "no upstream resolver for this name" is not a forwarder failure as far as its health bus is concerned, so nothing raises a warning while every query returns SERVFAIL. The app now asks the daemon's own resolver, on the refresh tick that already runs, whether it can still answer this node's own name; after a few silent ticks the redirect is taken off the device and the phone resolves names the way it did before Root Mode, and it goes back on by itself as soon as MagicDNS answers again. Every transition says which way it went and why in the ROOT tab of the Logs screen. The rules themselves are always built — only the two hooks that send traffic into them come and go — so re-arming is instant.
- **Public IPv6 through the TUN tunnel is now something you turn on.** TUN mode always gave the interface a global IPv6 address and, in full-tunnel mode, the `::/0` route, so Android believed the device had IPv6 and browsers tried AAAA first — while the SOCKS side answered "network is unreachable" for every public v6 address. Sites hung instead of falling back to IPv4. Tailnet IPv6 still rides the tunnel unconditionally and works in both routing modes; **Route all IPv6 through the tunnel** (Tunnel mode) now covers the rest of the internet's IPv6 and is off unless you turn it on. Turn it on only if your exit node really carries IPv6.
- **Turning on VPN (TUN) mode asks first.** Android hands the VPN slot to one app at a time, and enabling TUN was a single tap that silently revoked it from whatever held it — an ad blocker owning that slot simply went dark, with nothing to say why. Enabling TUN now confirms, the way Root already did; when another VPN is actually running the dialog says so (the platform does not name the app, so it does not pretend to) and it states the reverse too — the next app to take the slot will disconnect TailSocks the same way. Cancelling changes nothing, and turning TUN off still needs no confirmation.
- **The tunnel mode is a list of three options, not a tab strip.** A tab strip reads as three ways of looking at the same thing; the tunnel mode is not that — it decides how every packet leaves the phone, and switching it tears one tunnel down and brings another up. Proxy, VPN (TUN) and Root are now three rows, each saying in one line what it does, with the chosen one highlighted and its own settings expanding underneath. Root still goes through its warning dialog before anything is written.
- **Settings were rebuilt from six tabs into a list of sections.** Settings now opens as a plain list — Appearance & language, Account & connection, Tunnel mode, Local proxies, DNS, Censorship bypass, Sharing & access, Background & permissions, Backup & restore, Automation & API, Diagnostics & developer — and tapping one opens only that page, so you can see every category at once instead of scrolling a strip of six chips. Appearance & language sits first: it is the one category that is about the app rather than about the tunnel, and it is where you go when the app is in the wrong language.
- **Things that belong together are together now.** Proxy, VPN (TUN) and Root became one three-way choice under Tunnel mode, instead of a switch in one tab and a whole tab somewhere else; DNS options that were split across three tabs sit under DNS; ByeDPI moved next to the control-plane proxy it overrides; "Accept routes" and the device name joined the account, log verbosity and extra arguments joined Diagnostics, and the two separate backup/restore pairs became one Backup & restore section. Taildrive shares, Serve & Funnel, DNS diagnostics, Logs, Terminal, Netcheck and the Admin Console can now be opened straight from the section they belong to, the VPN option names the local SOCKS5 proxy it forwards into and links to it, and the battery-optimization row was dropped from Settings — it lives on the Permissions screen with the rest of them.
- **The profile backup file now carries the app-wide settings too** — proxy addresses, control-plane proxy and DPI bypass, TUN and Root Mode options, recovery and appearance — which until now survived only in the encrypted full backup. Files written by earlier versions still restore, a setting the file does not mention keeps its current value, and the automation secret, the API token, the node keys and device-bound entries are never written to it.

### Added

- **Apps can be excluded from Root Mode.** Root Mode changes routing and DNS for the whole device: every app's DNS is redirected into MagicDNS, and while an exit node is selected every app's traffic follows it — so one misbehaving tunnel took unrelated apps down with it (an app that could not resolve a single domain while Root Mode was on). The **Excluded apps** list — the same one VPN (TUN) mode already had, now shown for Root as well, so one list governs both tunnel modes — leaves the apps on it alone: their traffic leaves through Wi-Fi or mobile data, or through another VPN app when one is running, as if Root Mode were off, and they can still reach your tailnet by address. Their DNS is a weaker promise and worth reading about in the Root guide before relying on it: the exclusion only reaches an app that sends its own queries, and for an ordinary app Android's resolver sends them on its behalf, so they still go to MagicDNS. The list is the one you already edit for VPN mode, so whatever is on it now applies to both; leave it empty and Root Mode behaves exactly as before. On a kernel without the firewall match involved, the Logs screen says the DNS half could not be applied; the routing half is unaffected.
- **System-wide DNS switch for Root Mode**: DNS redirection to MagicDNS can now be turned off, so a DNS filtering app can keep the system resolver. It is no longer the only way out of a conflict with another VPN app — that case is handled on its own — but it is still the one switch that covers every app on the device, including the ones the per-app exclusion list cannot reach.
- **Ignore other VPNs** (Tunnel mode → Root): the root daemon marks its own sockets the way `VpnService.protect()` does for every VPN app, so another client on the phone can no longer swallow the daemon's connection to the control plane. This switch turns that off for the opposite case — deliberately running this tunnel inside another client — at the cost of double encapsulation.
- **Root Mode routing check**: A button in Diagnostics & developer that shows the live routing and firewall state of `tailscale0`, and failures now appear in the ROOT tab of the Logs screen instead of failing silently. It answers the questions a broken device actually raises: where each excluded app's packets go, where the local network goes, whether another VPN app holds the device and what it was recognised by, whether any rule was installed but is never evaluated, whether anything but TailSocks is marking packets, and how often the DNS rules are being hit.
...
- **LAN Access**: New switch in Local proxies that opens the SOCKS5 proxy, the HTTP proxy and the local DNS server to the local network (`0.0.0.0`) instead of this device only, so other devices can route through your tailnet. Shows the address to connect to and warns when the SOCKS5 proxy has no password.
...
- **Background revival**: Optionally checks every 15 minutes that the service is still alive and starts it again after a background kill. The check now asks for the "Alarms & reminders" permission when you turn it on — with it the check is allowed to start the service from the background, without it the check still runs, only later.
- **A refused restart is no longer invisible.** When the system stops the service and then refuses to let it back, a notification says so and reconnects in one tap — a tap is a start Android never refuses. It disappears as soon as the connection is up again.
- **A one-time pointer to autostart.** If the app opens and finds that the connection was taken down in the background and could not come back, a small dialog explains what happened and offers to open the autostart settings of your ROM (the app info page where that screen does not exist). Answer it with **Later** and it waits for the next such outage; answer it with **Don't ask** and it never comes back. The question now survives a language change, a rotation and a restart of the app instead of vanishing halfway through — it goes away when you answer it, not before.
- **Permissions screen** (Settings → Background & permissions → Permissions): every optional permission in one list — autostart, Alarms & reminders, battery optimization, notifications, installing unknown apps — each with why it matters, whether it is granted, and a button to the system screen that grants it. The status is rechecked every time you come back from those screens. Autostart is the one Android refuses to report, so it is shown as unknown rather than guessed at.
...
- **An exit node took the local network with it.** The catch-all that sends every unmarked local packet into the exit node also matched the router, a NAS, a printer and adb over Wi-Fi, so all of them disappeared the moment an exit node was selected. The daemon's table now carries throw routes for the subnets on your own exclusion list and for link-local, so those destinations fall through to Android's own rules and leave by the physical interface, while the exit node keeps everything else.
- **A carrier or a foreign tunnel living inside `100.64.0.0/10` was captured with no way out.** The rule that claims the tailnet range sat above every per-app exclusion and matched on the destination alone, so a mobile network handing out a CGNAT address — or another VPN client whose relay has one — had that traffic pulled into `tailscale0`. Any interface that is not ours and holds an address in that range now gets a throw route, and the lookup falls through to the system.
- **Root Mode's routing table could be one Android was already using.** The table number Root Mode wrote its tailnet routes into is also the number Android gives to a network once one of its interfaces reaches a certain index — which a phone that has cycled through enough Wi-Fi, mobile, tethering and tunnel interfaces reaches in ordinary use. Root Mode then wrote into a live network's table, and cleaning up emptied it. It uses a table of its own now, at boot as well as in the app; what older versions left in the old one is still removed on every start and every stop, entry by entry, never by emptying a table that may not be ours.
- **Root Mode: excluded apps were excluded by two mechanisms, one of which could not work.** A firewall mark applied on the way out arrives after the kernel has already chosen the route and the source address for the connection, so only the routing rule ever did anything; the mark is gone and is cleaned up on upgrade. That routing rule also assumed a fixed position in Android's own list of rules, and where the assumption was wrong the rule was accepted and then silently ignored, leaving the app inside the tunnel while the app list said otherwise. The position is now read off the device, the installed rule is read back, and where it cannot be installed at all the app is simply not excluded — said so in the Logs screen — instead of being left half-routed.
- **An excluded app is handed back to Android, not pinned to Wi-Fi.** Sending it to a physical table took it out of any other VPN client on the phone as well, which is how Chrome went dark with a DNS error while its tunnel client saw no requests at all. The rule now jumps over our own rules onto the platform's, so an excluded app ends up wherever Android says it belongs: inside the other client's tunnel when one is up, on the physical network when none is.
- **Root Mode: what "excluded" does to an app's DNS.** The excluded-apps list promised those apps the system resolver and for most of them never delivered: the firewall match involved identifies the process that sends the query, and on Android an ordinary app does not send its own — the platform resolver does, on its behalf. The carve-out therefore only ever covered apps that resolve for themselves (Chrome's built-in resolver, DoH/DoT clients, command-line tools), and for everything else the queries went to MagicDNS like every other app's. Nothing is claimed for it any more that it cannot do, the Root guide says which apps it covers, and **System-wide DNS via MagicDNS** remains the switch that gives the whole device its own resolver back.
- **The root daemon's own traffic was swallowed by another VPN client.** The root daemon has no `VpnService`, so nothing marked its sockets as protected: captured on a phone with a v2ray client running, that client sniffed the control-plane proxy's hostname out of our connection, tried to carry it through its own relay, could not resolve the relay because its queue was full of our retries, and took the device's networking down while our daemon reconnected in a tight loop. The daemon now sets the same protect bit the platform sets for every VPN app, so those packets leave by the physical network; **Ignore other VPNs** turns it off for the opposite case.
- **Root Mode claimed traffic that was only passing through.** The tailnet rules also matched packets forwarded through the phone — from a tethered device or a shared connection — and answered them with this device's tailnet address. They now apply only to what this device sends itself.
- **The daemon's state was left owned by root.** `tailscaled.state` and the profile data are written as root and rewritten on every login and prefs change, so once Root Mode had run, a later non-root daemon could not read the app's own state: it started from nothing and asked for a login while the node was still registered — an empty peer list on a working tailnet. The state directory is handed back to the app the moment the root daemon is stopped, which is the last point at which root is certainly available.
- **The installed rules and what the app believed about them could disagree.** "An apply has run" and "rules exist on this device" were one flag, so re-arming one disarmed the other; a failed cleanup still cleared the marker and orphaned the whole ruleset; and a failed apply left behind whatever it had already written with nothing owed to remove it. Those are separate now, and the rules are also re-applied when an app is installed or removed and when the excluded-apps list changes, instead of only when the connection does.
- **Root Mode: the daemon's own fallback resolvers could be redirected into MagicDNS.** The fallback resolvers listed in settings were excluded from the system-wide DNS redirect, but the ones the daemon falls back to by itself when that list is left empty were not, so their queries were sent back into MagicDNS and nothing outside the tailnet resolved. They are excluded now too.
- **Root Mode: the firewall mark collided with Android's own.** The mark used for tailnet traffic overwrote Android's routing mark, which broke routing when another VPN owned the default network. A dedicated mark bit is used instead, leaving the system's mark untouched.
...
- **Root Mode: exit nodes work, router health warning gone.** Selecting an exit node in Root Mode either did nothing or looped the tunnel's own traffic into `tailscale0`, and the daemon logged a permanent `router config failed` health warning: the desktop-Linux router was compiled in unchanged, with rules that assume a populated `main` table, netfilter on kernels that lack the nft `nat` chain, and marks that collide with Android's own. The daemon now marks its sockets (`0x2000000`) and only manages addresses and routes (table 52); the app routes unmarked local traffic through that table at priority 200 (`fwmark 0x0/0x2020000 iif lo lookup 52` when this app owns the device, or the same selector narrowed to one `uidrange` rule per carried app when it shares the phone with another VPN client), once it has established that the daemon marks its sockets; it also purges stale desktop rules and loosens `rp_filter` when needed. Subnet routes accepted with `--accept-routes` work in Root Mode for the same reason, and **Check Routing** now shows the complete rule set and the daemon's table.
...
- **Split DNS to a resolver on the tailnet works in Root Mode**: the policy-routing mark is applied after the kernel has chosen the source address, so the daemon's queries left `tailscale0` with the Wi-Fi address and never got an answer. A destination rule for the tailnet ranges now steers them into TailSocks' own routing table with the tailnet address as source.
- **DNS no longer dies when the exit node refuses to answer it.** With an exit node selected, the daemon sends every ordinary lookup to that node's DNS-over-HTTPS endpoint, and a node replies `403 Forbidden` whenever it does not (or no longer) count you as its exit-node client — which is exactly what happens for a few seconds around every exit-node switch. The phone was then left with no working DNS at all (`DNS_PROBE_STARTED` in the browser, `Response code: RCodeServerFailure` from `tailscale dns query`) even though the tunnel was healthy and the app's own fallback resolvers had been configured all along. When the resolver for a name that took the default route fails — a transport error, an answer that is not `200`, or a `SERVFAIL`/`REFUSED` reply — the daemon now retries that one query against the app's fallback resolvers and answers from there, naming the resolver that failed and the one that answered in the Logs screen (at most once every 30 seconds). Names handled by a split-DNS route, tailnet names and reverse lookups of tailnet addresses are never retried this way, so a resolver you pointed at on purpose still cannot leak to a public one. Set `TS_DNS_RESCUE=0` to switch the rescue off (patch 14).

#### Connection, background and updates
...
- **Keep the connection awake never did anything.** The switch — "Keep running in background" before the settings were rebuilt — was saved in one place and read from three others, so the wake lock was never taken — and when it was, it expired after ten minutes. It now holds for the whole session, which is what kept the connection dying overnight.
...
- **Changing the tunnel mode did nothing when the daemon was not this app's.** Switching tunnel mode, LAN access or the Root DNS redirect only restarted the service while the Go bridge in this process owned a daemon — which it does not in Root Mode, where the daemon is a separate root process the app attaches to. The change then sat in preferences until the connection was toggled by hand: turning Root Mode on from proxy mode left the old daemon running. The decision is now made on whether a connection is wanted at all.
- **Service did not come back after an update** unless **Keep the connection awake** was on: installing a new version kills the running service, and the package-replaced handler only restarted it when that option was set. It now resumes whenever the service was running before the install; the option still decides only whether it also starts after a reboot.
...
#### Account, daemon and settings plumbing
...
- **The "Extra arguments" field never did anything.** Whatever you typed there was handed to the Go bridge and dropped: nothing in the app ever passed it to the daemon, so the field was decoration. The flags are parsed now and applied to the running node — `--accept-routes`, `--accept-dns`, `--shields-up`, `--ssh`, `--exit-node-allow-lan-access`, `--report-posture`, `--hostname`, `--login-server`, `--advertise-routes`, `--advertise-tags`, `--advertise-exit-node`, each of them negatable with `--no-…` or `=false` — and they are applied on top of the app's own settings, so this is a real escape hatch and not a second copy of the settings screen. Anything the app cannot honour (an unknown flag, a Linux-only one, a value that does not parse) is now named in the Logs screen instead of vanishing, and a single bad flag no longer costs you the rest of your settings. Routes and tags are the exception worth knowing: the app rewrites those from its own Advertise Routes and Advertise Tags fields a moment after every start, so set them there.
...
- **The status card said nothing about Root Mode.** TUN mode has always labelled itself on the main card; a device carrying every packet through `tailscale0` looked identical to plain proxy mode, where nothing is routed at all. The card now reads "Active + Root" and says the system goes through `tailscale0`, and Root takes precedence over the TUN label, because with Root on the VpnService is not what carries the traffic even when that switch is also set.
- **The exit node could not be cleared when the peer list was empty.** The "no exit node" row lived inside the branch that draws the peer list, so a device whose peers had not loaded — or whose exit node stopped offering itself — showed "no exit nodes available" over a dashboard still reporting traffic routed through one, with no way to turn it off. The clear action is now offered in the empty state too, whenever an exit node is actually set.
- **The app was in one language and its dialogs in another.** The language was applied twice — the app wrapped each screen's context, and Android's own per-app locale governed dialog windows — and on Android 13 and newer the two could disagree. The platform owns it from 13 on, where it reaches every window; the app's own wrapper now covers only Android 12 and below, which have no per-app locale. Changing the language also no longer sends the settings screen into a loop of recreating itself.
- **More of the interface is translated**: Root Mode, proxy link import, Taildrive, received files and the Admin Console showed a number of messages only in English; they now follow the app language.
- **The back gesture showed a fake previous screen.** Every screen consumed the predictive back gesture in order to play its own animation, which suppressed Android's cross-activity animation — the one that actually shows the screen you are going back to. Screens whose back simply closes them (Terminal, DNS, Logs, Peers, Netcheck, Files, Serve, Taildrive, Excluded apps) no longer intercept it, so the system reveals the real screen underneath; the excluded-apps list now saves its selection when the screen closes instead of relying on the intercepted gesture. Screens that pop state inside themselves still draw the transition, but over a real destination — the previous screen where it can be rendered, otherwise the destination's icon and name, which until now were passed in and silently dropped — following the platform's own motion (down to 90 %, rounded corners, a slide away from the swiped edge), and a cancelled swipe springs back to rest instead of snapping. Settings no longer hand-animates back at all, so the system's own transition plays there.
- **Terminal ran out of memory in a long session**: the output was kept in full while the screen was open and only trimmed when leaving it, so a screen left running all day grew without limit — and a history file written by an older version was loaded whole, freezing the screen on the first open. The output is now trimmed to its tail as it grows, and saving the command history no longer writes to disk on the UI thread.
- Smaller layout fixes: the DNS test button no longer covers its own title, the excluded-apps header fits its bar, switch labels no longer collide with the controls they belong to, and the autostart question is readable — a checkbox and one emphasised action instead of three competing buttons.
...
- **Root Mode no longer rewrites the phone's SELinux policy at every start and every boot.** The fixed rule it applied opened the daemon socket to every app on the device, and named domains that are wrong on most of them, so it usually did not even fix what it was there for. The app now adds a single rule for the two domains actually involved, only after its own connection was refused, and writes it to the ROOT log.
- **Opening a received file no longer opens the whole data folder.** The link the app hands to a viewer or to the installer could point at any file it stores — the node private keys, the daemon socket, the file holding the proxy password. Received Taildrop files moved into a folder of their own, and that folder and the update download are now the only things a link can be made for; files received by earlier versions are moved there on the first start.
...
- **Unencrypted HTTP is now allowed only towards Taildrive inside the tunnel.** The exception for local addresses was left over from an earlier design and nothing in the app used it.
