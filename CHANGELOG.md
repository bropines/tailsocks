...
- **A one-time pointer to autostart.** If the app opens and finds that the connection was taken down in the background and could not come back, it shows a card explaining what happened, with a button that opens the autostart settings of your ROM (the app info page where that screen does not exist) and one that reconnects right away. Dismiss it and it stays away until the next update.
